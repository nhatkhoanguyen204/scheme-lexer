  ...
  (let ((first-token (read-token)))
    (cond ((eof-object? first-token) first-token) ; just return eof
          ((not (simple-datum? first-token))
           (error "not a simple datum:" first-token))
  ...
  (let ((first-token (read-token)))
    (cond ((eof-object? first-token) first-token) ; just return eof
          ((simple-datum? first-token)
           (error "not a compound datum:" first-token))
          (else (read-compound-datum-helper first-token))
    )
  )
)

; Reads a compound datum from standard input, where the compound
; datum's first token is first-token. Returns the Scheme
; representation of the datum. Raises an error if the compound datum
